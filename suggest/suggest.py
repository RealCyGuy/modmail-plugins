    ...
    async def suggest(self, ctx, *, suggestion):
        ...
        **Usage**:
        ...
        if str(ctx.author.id) not in self.banlist:
            ...
            await ctx.send(embed=discord.Embed(color=self.bot.error_color, title=f"You have been blocked, {ctx.author.mention}.", description=f"Reason: {self.banlist[str(ctx.author.id)]}"))
        ...
        **Examples:**
        ...
        if str(user.id) in self.banlist:
            embed = discord.Embed(
                ...
                title=f"{user.mention} is already blocked.",
            ...
            embed = discord.Embed(
                ...
                title=f"{user.mention} is now blocked.",
    ...
    async def unblock(self, ctx, user: discord.User):
        ...
        **Examples:**
        ...
        if str(user.id) not in self.banlist:
            embed = discord.Embed(
                colour=self.bot.error_color,
                title=f"{user.mention} is not blocked.",
                description=f"Reason: {self.banlist[str(user.id)]}",
            )
        else:
            self.banlist.pop(str(user.id))
            embed = discord.Embed(
                colour=self.bot.main_color, title=f"{user.mention} is now unblocked."
            )
